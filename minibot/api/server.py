...
def _parse_json_content(body: dict) -> tuple[str, list[str]]:
    ...
    media_dir = Path.home() / ".minibot" / "media" / "api"
...
async def _parse_multipart(request: web.Request) -> tuple[str, list[str], str | None, str | None]:
    ...
    while True:
        ...
        if part.name == "message":
            ...
            filename = f"{uuid.uuid4().hex[:12]}_{part.filename or 'upload.bin'}"
            dest = media_dir / filename
            dest.write_bytes(raw)
...
async def handle_chat_completions(request: web.Request) -> web.Response:
        ...
        async def _run() -> None:
            ...
            try:
                async with session_lock:
                    ...
                    await queue.put("Hello, this is a streaming response.")
                    await asyncio.sleep(1)
                    await queue.put("How can I help you today?")
                    await asyncio.sleep(1)
                    await queue.put(None)
            except Exception:
                stream_failed = True
    ...
    # -- non-streaming path --
    _FALLBACK = "I'm sorry, I couldn't generate a response. Please try again."
    ...
    try:
        async with session_lock:
            try:
                ...
                # Actual implementation needs to be adjusted based on agent_loop's API
                await asyncio.sleep(1)  # Simulate processing time
                response_text = "Hello, this is a non-streaming response. How can I help you today?"
...
def create_app(
    agent_loop, model_name: str = "minibot", request_timeout: float = 120.0
) -> web.Application:
